...
class Context {
  ...
  // 如果想自定义 IO，需在 avformat_open_input 之前设置 AVIOContext，可参见
   ...
   * @param type AVMEDIA_TYPE_VIDEO 初始化视频流相关参数，AVMEDIA_TYPE_AUDIO
   * 初始化音频流相关参数
   * @return 成功 true，失败 false
   */
  bool InitDecodeContext(int* stream_index, AVCodecContext** dec_ctx,
  ...
  std::future<void> heart_future_;  // 用于知悉更新 alive_micro_seconds_
                                    // 的线程是否退出
 ...
 public:
  ...
  AVStream* GetAudioStream() {
    return format_context_->streams[audio_stream_idx_];
  }
  AVStream* GetVideoStream() {
    return format_context_->streams[video_stream_idx_];
  }
  ...
  bool IsVideoPacket(const AVPacket* p) const {
    return p && p->stream_index == video_stream_idx_;
  }
  bool IsAudioPacket(const AVPacket* p) const {
    return p && p->stream_index == audio_stream_idx_;
  }
  ...
  int64_t CalcDelayTimeInMicroSecond(const util::AVFrameWrapper& frame) {
    ...
    return (time_point - ti.start) -
           (alive_micro_seconds_ - ti.corresponding_alive_micro_seconds_) +
           delta;
  }
