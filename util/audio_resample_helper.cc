...
bool AudioResampleHelper::Reset(uint64_t ic, int is, AVSampleFormat isf,
                                uint64_t oc, int os, AVSampleFormat of) {
  if (swr_ctx_.get() && swr_in_channel_layout_ == ic &&
      swr_in_sample_rate_ == is && swr_in_sample_fmt_ == isf &&
      swr_out_channel_layout_ == oc && swr_out_sample_rate_ == oc &&
...
bool AudioResampleHelper::Resample(AVFrameWrapper& wrapper, int out_rate,
                                   uint64_t out_channel_layout,
                                   AVSampleFormat out_fmt) {
  if (!Reset(wrapper->channel_layout, wrapper->sample_rate,
             AVSampleFormat(wrapper->format), out_channel_layout, out_rate,
             out_fmt)) {
    return false;
  }
  ...
  int ret = swr_convert_frame(swr_ctx_.get(), new_sample.GetRawPtr(),
                              wrapper.GetRawPtr());
  if (ret) {
