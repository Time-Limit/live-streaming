...
const std::string Neter::CA_FILE_PATH =
...
void Neter::TryAddReadTask(SessionPtr ptr) {
  if (false ==
      ptr->TestAndModifyEventFlag(Session::READ_ACCESS | Session::READ_PENDING,
                                  Session::READ_ACCESS, Session::READ_PENDING,
                                  Session::EMPTY_EVENT_FLAG)) {
    Log::Debug("Neter::TryAddReadTask, unexcept event flag");
    return;
  }

  if (false == Neter::GetInstance().threadpool.AddTask(
                   TaskPtr(new SessionReadTask(ptr)))) {
...
void Neter::TryAddWriteTask(SessionPtr ptr) {
  if (false ==
      ptr->TestAndModifyEventFlag(
          Session::WRITE_ACCESS | Session::WRITE_PENDING | Session::WRITE_READY,
          Session::WRITE_ACCESS | Session::WRITE_READY, Session::WRITE_PENDING,
          Session::EMPTY_EVENT_FLAG)) {
  ...
  if (false == Neter::GetInstance().threadpool.AddTask(
                   TaskPtr(new SessionWriteTask(ptr)))) {
...
void Neter::Wait(time_t timeout) {
  ...
  for (; begin < end; ++begin) {
    ...
    if (event.events & (~(EPOLLIN | EPOLLOUT))) {
      Log::Error("Neter::Wait, unhandle event, event=", std::hex, event.events,
                 std::dec);
      it->second->Close();
    }
  }
  ...
  if (tmp_ready_close_session_list.size()) {
    Log::Trace("Neter::Wait, ", tmp_ready_close_session_list.size(),
               " sessions will close!");
  }
...
Neter::Neter()
    : threadpool(THREAD_COUNT, ThreadPool::PT_XT_TO_XQ,
  ...
  if (epoll_instance_fd == -1) {
    Log::Error("Neter::Neter, create epoll instance failed, info=",
               strerror(errno));
    return;
  }
...
Neter::~Neter() {
  if (epoll_instance_fd != -1) {
    ...
    if (res == -1) {
      Log::Error("Neter::~Neter, close epoll instance failed, info=",
                 strerror(errno));
    }
  }
}
