...
class Octets {
  ...
  void* base;
  Rep* rep() const {
    return reinterpret_cast<Rep*>(base) - 1;
  }
  void unique() {
    base = rep()->unique();
  }
 ...
 public:
  ...
  Octets(const void* x, const void* y)
      : base(Rep::create((char*)y - (char*)x)->data()) {
    size_t size = (char*)y - (char*)x;
    memcpy(base, x, size);
    rep()->len = size;
  }
  Octets(const Octets& x) : base(x.base) {
    rep()->retain();
  }
  ...
  void dump() {
    ...
    for (size_t i = 0; i < size(); i++) fprintf(stderr, "%02x ", *p++);
    fprintf(stderr, "\n");
  }
};
...
class OctetsStream {
 ...
 public:
  ...
  explicit OctetsStream(const Octets& d)
      : data(d), start_cursor(0), cursor(0) {}

  const Octets& GetData() const {
    return data;
  }

  void push_back(const void* x, const void* y) {
    data.insert(data.end(), x, y);
  }
  void push_back(const void* x, size_t s) {
    data.insert(data.end(), x, ((const char*)x) + s);
  }
  OctetsStream& operator<<(const Protocol& d);
  OctetsStream& operator<<(const unsigned char& d) {
    return push_bytes(d);
  }
  OctetsStream& operator<<(const char& d) {
    return push_bytes(d);
  }
  OctetsStream& operator<<(const unsigned short& d) {
    return push_bytes(d);
  }
  OctetsStream& operator<<(const short& d) {
    return push_bytes(d);
  }
  OctetsStream& operator<<(const unsigned int& d) {
    return push_bytes(d);
  }
  OctetsStream& operator<<(const int& d) {
    return push_bytes(d);
  }
  OctetsStream& operator<<(const unsigned long& d) {
    return push_bytes(d);
  }
  OctetsStream& operator<<(const int64_t& d) {
    return push_bytes(d);
  }
  OctetsStream& operator<<(const float& d) {
    return push_bytes(d);
  }
  OctetsStream& operator<<(const double& d) {
    return push_bytes(d);
  }
  OctetsStream& operator<<(const Octets& d) {
    return push_bytes(d);
  }
  ...
  OctetsStream& operator>>(unsigned char& d) {
    return pop_bytes(d);
  }
  OctetsStream& operator>>(char& d) {
    return pop_bytes(d);
  }
  OctetsStream& operator>>(unsigned short& d) {
    return pop_bytes(d);
  }
  OctetsStream& operator>>(short& d) {
    return pop_bytes(d);
  }
  OctetsStream& operator>>(unsigned int& d) {
    return pop_bytes(d);
  }
  OctetsStream& operator>>(int& d) {
    return pop_bytes(d);
  }
  OctetsStream& operator>>(unsigned long& d) {
    return pop_bytes(d);
  }
  OctetsStream& operator>>(int64_t& d) {
    return pop_bytes(d);
  }
  OctetsStream& operator>>(float& d) {
    return pop_bytes(d);
  }
  OctetsStream& operator>>(double& d) {
    return pop_bytes(d);
  }
  OctetsStream& operator>>(Octets& d) {
    return pop_bytes(d);
  }
