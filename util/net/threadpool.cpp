...
void ThreadPool::Stop() {
  Log::Trace("ThreadPool::Stop, the number of TaskQueue is ",
             taskqueues.size());
...
void Thread::exit_cur_thread(int) {
  Log::Error("Thread::exit_cur_thread, tid=", std::hex, pthread_self(),
             std::dec);
  pthread_exit(nullptr);
}

void Thread::notify_task_quit(int) {
  Log::Error("Thread::notify_task_quit, tid=", std::hex, pthread_self(),
             std::dec);
  pthread_exit(nullptr);
}
