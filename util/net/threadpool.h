...
class ClearTaskQueueTask : public Task {
 ...
 public:
  ClearTaskQueueTask(TaskQueuePtr ptr)
      : Task(), stop_flag(false), task_queue_ptr(ptr) {}
...
class ThreadPool {
 ...
 public:
  explicit ThreadPool(
      size_t _thread_size, POOL_TYPE type,
      DistributeFunc _distribute = DistributeFunc(DefaultDistributeFunc))
    ...
    try {
      if (type == PT_XT_TO_XQ) {
        ...
        for (size_t i = 0; i < _thread_size; ++i) {
          threads.push_back(
              Thread(TaskPtr(new ClearTaskQueueTask(taskqueues[i]))));
        }
        ...
        for (size_t i = 0; i < _thread_size; ++i) {
          threads.push_back(
              Thread(TaskPtr(new ClearTaskQueueTask(taskqueues[0]))));
        }
      } else {
        throw;
      }
