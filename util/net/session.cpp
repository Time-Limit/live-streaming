...
void Neter::Session::Close() {
  if (TestAndModifyEventFlag(CLOSE_READY, EMPTY_EVENT_FLAG, CLOSE_READY,
                             EMPTY_EVENT_FLAG)) {
...
bool Neter::Session::TestAndModifyEventFlag(event_flag_t test,
                                            event_flag_t except,
                                            event_flag_t set,
                                            event_flag_t clear) {
  SpinLockGuard guard(event_flag_lock);
...
void Neter::Session::ConnectorWriteFunc() {
  ...
  if (ret != 0 || val != 0 || len != sizeof(int)) {
    Log::Error("Neter::Session::ConenctorWriteFunc, ip=", GetIP(),
               ", port=", GetPort(), ", ret=", ret, ", val=", val,
               ", len=", len, ", info=", strerror(errno));
    Close();
    return;
  }
  ...
  Log::Trace("Neter::Session::ConnectorWriteFunc, connect success, ip=",
             GetIP(), ", port=", GetPort());
...
void Neter::Session::AcceptorReadFunc() {
  ...
  for (;;) {
    new_fd = accept(GetFD(), (struct sockaddr*)&accept_addr,
    ...
    if (new_fd > 0) {
      ...
      SessionPtr ptr(new Session(Neter::GetInstance().GenerateSessionID(),
                                 Session::EXCHANGE_SESSION, new_fd));
      ...
      Log::Trace("Neter::Session::AcceptorReadFunc, client's ip: ",
                 ptr->GetIP(), ", port: ", ptr->GetPort());
      epoll_event ev;
      ...
      if (errno == EAGAIN) {
        ...
        Log::Error("Neter::Session::AcceptorReadFunc, accept failed, info=",
                   strerror(errno));
  ...
  for (;;) {
    ...
    if (percnt > 0) {
      ...
      if (errno == EINTR) {
        ...
        Log::Error("Neter::Session::ExchangerReadFunc, read failed, info=",
                   strerror(errno));
        Close();
        return;
      }
...
void Neter::Session::Read(SessionPtr ptr) {
  if (false == ptr->TestAndModifyEventFlag(
                   CLOSE_READY | READ_ACCESS | READ_PENDING,
                   READ_ACCESS | READ_PENDING, Session::EMPTY_EVENT_FLAG,
                   READ_PENDING | READ_ACCESS)) {
    return;
  }
...
void Neter::Session::ExchangerWriteFunc() {
  ...
  while (tmp_send_data_list.size() > 0) {
    ...
    int per_cnt = write(
        fd, static_cast<const char*>(data.begin()) + cursor_of_first_send_data,
        data.size() - cursor_of_first_send_data);
    if (per_cnt > 0) {
      ...
      if (errno == EINTR) {
        ...
        Log::Error("Neter::Session::ExchangerWriteFunc, info=",
                   strerror(errno));
        Close();
        return;
      }
...
void Neter::Session::Write(SessionPtr ptr) {
  if (false == ptr->TestAndModifyEventFlag(
                   CLOSE_READY | WRITE_ACCESS | WRITE_READY | WRITE_PENDING,
                   WRITE_ACCESS | WRITE_READY | WRITE_PENDING, EMPTY_EVENT_FLAG,
                   WRITE_READY | WRITE_PENDING)) {
    return;
  }
